...
/// Specifies the order in which nodes should be visited during traversal
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum VisitOrder {
    /// Visit the current node before its children (traditional visitor pattern)
    PreOrder,
    /// Visit the current node after its children (useful for bottom-up analysis)
    PostOrder,
}
...
/// Post-order visitor trait that visits children before the current node
///
/// This trait extends the base `Visitor` trait to provide post-order traversal.
/// It reuses the existing walk functions but calls them in the default implementations
/// so that children are processed before the current node.
///
/// # Example
///
/// ```rust
/// use tlang_hir::{PostOrderVisitor, Visitor, hir};
/// use tlang_hir::visit::walk_expr;
///
/// struct MyPostOrderVisitor {
///     depth: usize,
/// }
///
/// impl<'hir> Visitor<'hir> for MyPostOrderVisitor {
///     // Implement base visitor methods as needed
/// }
///
/// impl<'hir> PostOrderVisitor<'hir> for MyPostOrderVisitor {
///     fn visit_expr(&mut self, expr: &'hir mut hir::Expr, ctx: &mut Self::Context) {
///         // Children are automatically visited first due to default implementation
///         walk_expr(self, expr, ctx);
///         // Then process the current expression
///         println!("Processing expr at depth {}: {:?}", self.depth, expr.kind);
///     }
/// }
/// ```
pub trait PostOrderVisitor<'hir>: Visitor<'hir> {
    fn visit_module(&mut self, module: &'hir mut hir::Module, ctx: &mut Self::Context) {
        walk_module(self, module, ctx);
    }
