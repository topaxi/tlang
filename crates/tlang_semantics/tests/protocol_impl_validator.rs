...
#[test]
fn error_on_parameter_type_mismatch() {
    let diagnostics = analyze_diag!(indoc! {"
        protocol Hash {
    ...
    let type_errors: Vec<_> = diagnostics
        .iter()
        .filter(|d| d.message().contains("type mismatch for parameter"))
        .collect();
    assert_eq!(
        type_errors.len(),
        1,
        "expected one parameter type error, got {diagnostics:#?}"
    );
    assert!(type_errors[0].is_error());
    assert!(type_errors[0].message().contains("Hash::hash"));
    assert!(type_errors[0].message().contains("i64"));
    assert!(type_errors[0].message().contains("bool"));
}

#[test]
fn no_error_when_parameter_types_match() {
    let diagnostics = analyze_diag!(indoc! {"
        protocol Hash {
            fn hash(self, seed: i64) -> i64
        }
    ...
    assert!(
        ...
        "expected no type mismatch errors, got {type_errors:#?}"
    );
}

#[test]
fn no_error_when_impl_omits_type_annotation() {
    // The protocol has typed params but the impl omits them — that is allowed;
    // the types will be checked at a later stage.
    let diagnostics = analyze_diag!(indoc! {"
        protocol Hash {
            fn hash(self, seed: i64) -> i64
        }
    ...
    assert!(
        type_errors.is_empty(),
        "expected no type mismatch when impl omits annotation, got {type_errors:#?}"
    );
}
...
#[test]
fn error_on_return_type_mismatch() {
    let diagnostics = analyze_diag!(indoc! {"
        protocol Stringify {
    ...
    let ret_errors: Vec<_> = diagnostics
        .iter()
    ...
    assert_eq!(
        ret_errors.len(),
        1,
        "expected one return type error, got {diagnostics:#?}"
    );
    assert!(ret_errors[0].is_error());
    assert!(ret_errors[0].message().contains("Stringify::to_string"));
    assert!(ret_errors[0].message().contains("string"));
    assert!(ret_errors[0].message().contains("i64"));
}

#[test]
fn no_error_when_return_types_match() {
    let diagnostics = analyze_diag!(indoc! {"
        protocol Stringify {
            fn to_string(self) -> string
        }
    ...
    assert!(
        ret_errors.is_empty(),
        "expected no return type errors, got {ret_errors:#?}"
    );
}

#[test]
fn no_error_when_impl_omits_return_type_annotation() {
    let diagnostics = analyze_diag!(indoc! {"
        protocol Stringify {
            fn to_string(self) -> string
        }
    ...
    assert!(
        ret_errors.is_empty(),
        "expected no return type errors when annotation omitted, got {ret_errors:#?}"
    );
}

// ── generic protocol type params should not trigger type errors ───────────────

#[test]
fn no_error_for_generic_protocol_type_param_in_return() {
    // protocol Into<T> { fn into(self) -> T }
    // impl Into<i64> for Str { fn into(self) -> i64 { ... } }
    // `T` is a type param → skip comparison, no error.
    let diagnostics = analyze_diag!(indoc! {"
        protocol Into<T> {
            fn into(self) -> T
        }
    ...
    assert!(
        errors.is_empty(),
        "expected no return type errors for generic protocol, got {errors:#?}"
    );
}

#[test]
fn no_error_for_generic_protocol_type_param_in_parameter() {
    // `T` is a type param of the protocol → its occurrence in the parameter
    // type should not be compared against the concrete `i64` in the impl.
    let diagnostics = analyze_diag!(indoc! {"
        protocol From<T> {
            fn from(value: T)
        }
    ...
    assert!(
        errors.is_empty(),
        "expected no type errors for generic protocol param, got {errors:#?}"
    );
}
