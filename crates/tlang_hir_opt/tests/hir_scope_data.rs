...
fn test_hir_scope_data_counter_higher_order_function() {
    ...
    let create_counter_scope = scope_data.iter().find(|(name, _, _)| name == "fn_create_counter");
    assert!(create_counter_scope.is_some(), "create_counter function should have scope data");
    let (_, locals, _) = create_counter_scope.unwrap();
    assert!(*locals > 0, "create_counter should have locals (count variable, counter function)");

    // Find counter function
    let counter_scope = scope_data.iter().find(|(name, _, _)| name == "fn_counter");
    assert!(counter_scope.is_some(), "counter function should have scope data");
    let (_, locals, upvars) = counter_scope.unwrap();
    assert!(*locals > 0 || *upvars > 0, "counter should have locals or upvars (for count access)");
    ...
    assert!(*locals > 0, "module should have locals for global variables");
}
...
fn test_hir_scope_data_simple_enum() {
    ...
    let get_direction_scope = scope_data.iter().find(|(name, _, _)| name == "fn_get_direction");
    assert!(get_direction_scope.is_some(), "get_direction function should have scope data");
    ...
    assert!(*locals > 0 || *upvars > 0, "get_direction should have scope data for enum access");

    // Module should have scope data
    let module_scope = scope_data.iter().find(|(name, _, _)| name == "module");
    assert!(module_scope.is_some(), "module should have scope data");
    let (_, locals, _) = module_scope.unwrap();
    assert!(*locals > 0, "module should have locals for global variables and enum");
}
...
fn test_hir_scope_data_tagged_enum_pattern_matching() {
    ...
    let unwrap_or_scopes: Vec<_> = scope_data.iter()
        .filter(|(name, _, _)| name.starts_with("fn_unwrap_or"))
        .collect();
    assert!(!unwrap_or_scopes.is_empty(), "unwrap_or functions should have scope data");
    
    // Each unwrap_or function should have locals for pattern matching
    for (_, locals, upvars) in &unwrap_or_scopes {
        assert!(*locals > 0 || *upvars > 0, "unwrap_or functions should have scope data for parameters");
    }

    // Find process_option function
    let process_option_scope = scope_data.iter().find(|(name, _, _)| name == "fn_process_option");
    assert!(process_option_scope.is_some(), "process_option function should have scope data");
    let (_, locals, upvars) = process_option_scope.unwrap();
    assert!(*locals > 0 || *upvars > 0, "process_option should have scope data for parameters and function calls");
...
fn test_hir_scope_data_nested_functions_with_closures() {
    ...
    let outer_scope = scope_data.iter().find(|(name, _, _)| name == "fn_outer_function");
    let middle_scope = scope_data.iter().find(|(name, _, _)| name == "fn_middle_function");
    let inner_scope = scope_data.iter().find(|(name, _, _)| name == "fn_inner_function");
    
    assert!(outer_scope.is_some(), "outer_function should have scope data");
    assert!(middle_scope.is_some(), "middle_function should have scope data");
    assert!(inner_scope.is_some(), "inner_function should have scope data");
    ...
    assert!(*outer_locals > 0, "outer_function should have locals for parameters and variables");
    
    // Middle function should have locals and potentially upvars for accessing outer scope
    assert!(*middle_locals > 0, "middle_function should have locals for parameters and variables");
    
    // Inner function should have upvars for accessing middle and outer scope variables
    assert!(*inner_locals > 0 || *inner_upvars > 0, "inner_function should have scope data for closure variables");
}
...
fn test_hir_scope_data_recursive_function() {
    ...
    let factorial_scopes: Vec<_> = scope_data.iter()
        .filter(|(name, _, _)| name.starts_with("fn_factorial"))
        .collect();
    assert!(!factorial_scopes.is_empty(), "factorial functions should have scope data");
    
    // Each factorial function should have locals for parameters
    for (_, locals, upvars) in &factorial_scopes {
        assert!(*locals > 0 || *upvars > 0, "factorial functions should have scope data for parameters and self-reference");
    }
    ...
    assert!(module_scope.is_some(), "module should have scope data");
    let (_, locals, _) = module_scope.unwrap();
    assert!(*locals > 0, "module should have locals for result variable and factorial functions");
}
...
fn test_hir_scope_data_simple_function_exact_locals() {
    ...
    // - local_var1 (slot 3)
    // - local_var2 (slot 4) 
    let simple_function_scope = scope_data.iter().find(|(name, _, _)| name == "fn_simple_function");
    assert!(simple_function_scope.is_some(), "simple_function should have scope data");
    let (_, locals, upvars) = simple_function_scope.unwrap();
    
    // Should have at least 5 locals: function + 2 params + 2 local vars
    assert!(*locals >= 5, "simple_function should have at least 5 locals, got {}", locals);
    assert_eq!(*upvars, 0, "simple_function should have 0 upvars, got {}", upvars);
    
    // Module should have locals for simple_function and result
    let module_scope = scope_data.iter().find(|(name, _, _)| name == "module");
    assert!(module_scope.is_some(), "module should have scope data");
    let (_, locals, upvars) = module_scope.unwrap();
    assert!(*locals >= 2, "module should have at least 2 locals (simple_function, result), got {}", locals);
