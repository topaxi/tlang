...
impl LoweringContext {
    ...
    fn lower_call_expr(&mut self, node: &ast::node::CallExpression) -> hir::CallExpression {
        ...
        {
            if let Some(ctx) = self.protocol_dispatch_ctx.as_ref() {
                ...
                let is_self_receiver =
                    if let ast::node::ExprKind::Path(path) = &base.kind {
                        if path.segments.len() == 1
                            && path.segments[0].as_str() == kw::_Self
                        {
                            ...
                            match path.res {
                                ast::node::Res::Def(node_id) => {
                                    node_id == ctx.self_param_node_id
                                }
                ...
                if is_self_receiver && ctx.method_names.contains(field.as_str()) {
                    ...
                    let method_segment_name =
                        if self.has_multi_arity_fn(method_str, total_arity) {
